...
void PhysicsEngine::setSceneOuterBoundary(float x1, float z1, float x2,
                                          float z2) {
  outerBoundary = glm::vec4(x1, z1, x2, z2);
}

void PhysicsEngine::setSceneInnerBoundary(float x1, float y1, float z1,
...
void PhysicsEngine::outCollisionTest(glm::vec3 &cameraPos,
                                     glm::vec3 &targetPos) {
  outCollisionTestXZ(outerBoundary[0], outerBoundary[1], outerBoundary[2],
                     outerBoundary[3], cameraPos, targetPos);
}

void PhysicsEngine::outCollisionTestXZ(float x1, float z1, float x2, float z2,
                                       glm::vec3 &cameraPos,
                                       glm::vec3 &targetPos) {
  ...
  float distance =
      sqrt((cameraPos[0] - targetPos[0]) * (cameraPos[0] - targetPos[0]) +
           (cameraPos[2] - targetPos[2]) * (cameraPos[2] - targetPos[2]));

  if (distance <= 2.0f) {
    cameraPos[0] =
        2.0f * (cameraPos[0] - targetPos[0]) / distance + targetPos[0];
    cameraPos[2] =
        2.0f * (cameraPos[2] - targetPos[2]) / distance + targetPos[2];
  }
  bool flag = false;
  ...
  if (flag) {
    ...
    if (distance <= 2.0f) {
      targetPos[0] =
          2.0f * (targetPos[0] - cameraPos[0]) / distance + cameraPos[0];
      targetPos[2] =
          2.0f * (targetPos[2] - cameraPos[2]) / distance + cameraPos[2];
    }
  }
}
...
bool insideTheCollider(glm::vec3 _cameraPos, glm::vec3 _innerMin,
                       glm::vec3 _innerMax) {
...
void PhysicsEngine::updateCameraVertMovement(glm::vec3 &cameraPos,
  ...
  for (int i = 0; i < innerBoundaryMin.size(); i++) {
    ...
    if (insideTheCollider(cameraPos, innerBoundaryMin[i],
                          innerBoundaryMax[i])) {
      ...
      if (cameraPos.y >= innerBoundaryMin[i][1] &&
          cameraPos.y - HeroHeight <= innerBoundaryMin[i][1]) {
...
void PhysicsEngine::inCollisionTest(glm::vec3 &cameraPos,
                                    glm::vec3 &targetPos) {

  for (int i = 0; i < innerBoundaryMin.size(); i++) {
    inCollisionTestWithHeight(innerBoundaryMin[i][0], innerBoundaryMin[i][1],
                              innerBoundaryMin[i][2], innerBoundaryMax[i][0],
                              innerBoundaryMax[i][1], innerBoundaryMax[i][2],
                              cameraPos, targetPos);
  }
}

void PhysicsEngine::inCollisionTestWithHeight(float x1, float y1, float z1,
                                              float x2, float y2, float z2,
                                              glm::vec3 &cameraPos,
                                              glm::vec3 &targetPos) {
...
void PhysicsEngine::inCollisionTestXZ(float x1, float z1, float x2, float z2,
                                      glm::vec3 &cameraPos,
                                      glm::vec3 &targetPos) {
  const float d = 2.0f;
  float tarX = targetPos[0], camX = cameraPos[0], tarZ = targetPos[2],
        camZ = cameraPos[2];
  float len =
      sqrt((camX - tarX) * (camX - tarX) + (camZ - tarZ) * (camZ - tarZ));
